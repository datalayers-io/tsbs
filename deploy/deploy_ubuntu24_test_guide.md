# Datalayers + FoundationDB 三节点部署测试指南（内部 · 170/172/180 · Ubuntu 24.04 容器）

内部测试用：在 170 / 172 / 180 三台机器上用 `ubuntu:24.04` 容器（`--network host`）模拟，
验证 **Ubuntu 24.04 环境**的部署。端口使用非默认值，避免与机器上已有服务冲突。

> 连接用户：170 用 `nsc@192.168.1.170`，172 用 `nsq@192.168.1.172`，180 本机为 `nsc`
> （180 无需 ssh，直接在本机执行）。docker 命令若提示权限不足，前面加 `sudo`。
> **Bundle 须为 ubuntu 版**：`fdb_pkgs/` 放 amd64 deb，datalayers/dlsql/dldump 为 ubuntu24 所构建
...
```
poc_bundle.tar.gz
├── datalayers            # Datalayers 服务端可执行文件（与镜像 glibc 匹配）
├── dlsql                 # dlsql 命令行工具
...
# 0.2 起三个 ubuntu:24.04 容器（--network host）
sudo docker run -d --name node3 --network host --ulimit nofile=65535:65535 ubuntu:24.04 sleep 1000000
ssh nsc@192.168.1.170 'docker run -d --name node1 --network host --ulimit nofile=65535:65535 ubuntu:24.04 sleep 1000000'
ssh nsq@192.168.1.172 'docker run -d --name node2 --network host --ulimit nofile=65535:65535 ubuntu:24.04 sleep 1000000'
...
```bash
# 装依赖 + 解压（adduser 是 foundationdb-clients deb 的依赖）
apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y curl iproute2 procps hostname ca-certificates adduser python3 python3-pip
mkdir -p /root/deploy && cd /root && tar -xzf poc_bundle.tar.gz -C /root/deploy
ls /root/deploy

# Python 依赖（python 客户端示例用；requirements.txt 在 examples 仓库 python/ 目录，已按 el7 适配，ubuntu 同样适用）
...
pip3 install --only-binary=:all: -i https://mirrors.aliyun.com/pypi/simple/ -r /root/examples/python/requirements.txt
...
模拟客户真实环境，在 ubuntu:24.04 容器内 clone、编译并跑 tsbs smoke 测试：

```bash
# 5.1 安装构建工具 + Go
apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y make gcc git
...
| 现象 | 处理 |
|------|------|
| 二进制报缺共享库 / glibc 不匹配 | 用 `ldd ./datalayers` 排查；确认二进制与镜像 glibc 匹配（>2.39 则换 ubuntu:26.04） |
| `datalayers` 报找不到 `libfdb_c.so` | 容器内 `export LD_LIBRARY_PATH=/usr/lib` 后启动 |
| `fdbmonitor` 报 inotify 超限 | 宿主机 `sysctl -w fs.inotify.max_user_instances=1000000` |
| `foundationdb-server` deb 安装 postinst 报错 | 属预期（容器内自动建库竞态）；二进制已装好，脚本会继续，重跑即可 |
| `foundationdb-clients` deb 报依赖 `adduser` | 容器内 `apt-get install -y adduser` 后再跑 deploy_fdb.sh |
