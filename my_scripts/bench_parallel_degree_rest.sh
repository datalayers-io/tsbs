#!/usr/bin/env bash
#
# bench_parallel_degree_rest.sh - Probe optimal parallel_degree for all TSBS
# queries EXCEPT the double-groupby family (already probed).
#
# Usage:
#   bash bench_parallel_degree_rest.sh <DATALAYERS_REPO> <SERVER_PORT> [large]
#
# Output: <script_dir>/optimal_parallel_degree_rest.txt
#
set -euo pipefail
...
TMPDIR="${SCRIPT_DIR}/.bench_parallel_rest_$$"
OUTFILE="${SCRIPT_DIR}/optimal_parallel_degree_rest.txt"

COPIES=5
SAMPLES=3

DEGREES=(1 2 4 8 16 32 64 128)
...
echo "" >> "$OUTFILE"
echo "# Recommended parallel_degree per query except double-groupby (scenario: ${SCENARIO})" >> "$OUTFILE"
